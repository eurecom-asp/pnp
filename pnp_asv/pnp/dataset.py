...
class ConditionalDataset(torch.utils.data.Dataset):
  ...
  def __getitem__(self, idx):
    ...
    # -19-198-19-198-0000.flac.spec.npy -> 19/198/19-198-0000.spec.npy
...
class UnconditionalDataset(torch.utils.data.Dataset):
  def __init__(self, path, adv_path):
    super().__init__()
    self.filenames = []
    self.adv_filenames = []
    path = path[0] if isinstance(path, list) else path
    for adv_root, dirs, files in os.walk(adv_path):
      root = adv_root.replace(adv_path, path)
      for file in files:
        if file.endswith('.wav') or file.endswith('.flac'):
          self.adv_filenames.append(os.path.join(adv_root, file))
  ...
  def __getitem__(self, idx):
    ...
    adv_filename = self.adv_filenames[idx] 
    spec_filename = f'{audio_filename}.spec.npy'
    signal, _ = torchaudio.load(audio_filename)
    adv_signal, _ = torchaudio.load(adv_filename)
    return {
        'audio': signal[0],
        'spectrogram': None,
        'adv_audio': adv_signal 
    }
...
class Collator:
  ...
  def collate(self, minibatch):
    ...
    adv_audio = np.stack([record['adv_audio'] for record in minibatch if 'adv_audio' in record])

    if self.params.unconditional:
        return {
            'audio': torch.from_numpy(audio),
            'spectrogram': None,
            'adv_audio': torch.from_numpy(adv_audio),
        }
...
def from_path(data_dirs, params, is_distributed=False):
  if params.unconditional:
    dataset = UnconditionalDataset(data_dirs,adv_path=params.adv_path)
  else:#with condition
